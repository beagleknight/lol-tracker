...
test.describe("API routes", () => {
  ...
  test("/api/export/matches — returns CSV with correct headers", async ({ request }) => {
    ...
    expect(lines[0]).toBe(
      "Match ID,Date,Result,Champion,Matchup,Keystone,Kills,Deaths,Assists,CS,CS/min,Duration,Gold,Vision Score,Queue ID,Reviewed,Comment,VOD URL,Duo Partner Champion",
    );
  });
