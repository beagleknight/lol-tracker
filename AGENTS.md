...
**NEVER push a modified `package-lock.json` without verifying it with `npx -y npm@10 ci`.** This project runs Node 22 / npm 10 in CI, but the local environment uses Node 24 / npm 11. Lockfiles generated by npm 11 silently break `npm ci` in CI.

**Pre-push checklist** (when `package-lock.json` is staged):
1. If you did NOT change dependencies in `package.json`, **restore the lockfile from main**: `git checkout origin/main -- package-lock.json`
2. If you DID change dependencies, regenerate the lockfile with CI's npm version:
   ```bash
   git checkout origin/main -- package-lock.json
   npx -y npm@10 install --package-lock-only
   ```
3. **ALWAYS verify before pushing**: `npx -y npm@10 ci` — if this fails, DO NOT push.
