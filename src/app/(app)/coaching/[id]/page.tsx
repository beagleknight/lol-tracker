...
export default async function CoachingDetailPage({
  ...
  const [linkedMatchRows, items, ddragonVersion, allHighlights] =
    await Promise.all([
      db
        .select({
          id: matches.id,
  ...
  if (session.status === "completed") {
    ...
    const nextSession = await db.query.coachingSessions.findFirst({
      where: and(
        eq(coachingSessions.userId, user.id),
        gt(coachingSessions.date, session.date)
      ),
    ...
    if (progressMatches.length > 0) {
      ...
      for (const h of progressHL) {
        if (!progressHighlightsByMatch[h.matchId])
          progressHighlightsByMatch[h.matchId] = [];
